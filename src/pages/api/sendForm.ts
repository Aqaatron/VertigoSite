...
async function sendToMax(name: string, phone: string, comment: string, pageUrl: string) {
    ...
    const MAX_CHAT_ID = '-74492045124963';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        // Автоматически получаем адрес страницы из заголовка Referer
        const pageUrl = req.headers.referer || 'не указан (Referer отсутствует)';
        ...
        // Отправляем уведомление в MAX с адресом страницы
